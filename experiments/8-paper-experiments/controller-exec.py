...
output_log_file_name = functions.get_project_root()+'/logs/exp-8-2-controller-exec-dynamic.csv'
    ...
    for cb_cont in cb_controller:
        for retry_cont in retry_controller:
            if traffic != 'spike':
                ...
                functions.deploy_dynamic_loadgenerator(capacity=19, spikes=40, duration=loadgenerator_sleep_time)
                if cb_cont == 0:
                    ...
                    if retry_cont == 0:
                        ...
                        for retry_attempt in retry_attempts:
                            for retry_interval in retry_intervals:
                                cb_controllers = func.initialize_cb_controllers(service_list=services_list)
                                log_start_time = int(time.time() * 1000)

                                func.deploy_same_retrys(service_list=services_list, retry_attempt= retry_attempt, retry_interval=retry_interval)
                                while int(time.time() * 1000) < log_start_time + (experiment_duration * 1000):
                                    monitoring_data = func.monitor_services(services_list)
                                    for service in services_list:
                                    ...
                                    time.sleep(5)
                                    # func.delete_same_cbs(service_list=services_list)
                                func.delete_retrys(service_list=services_list, retry_attempt=retry_attempt, retry_interval=retry_interval)
                                log_end_time = int(time.time() * 1000)
                                log_dict = {
                                    "traffic": traffic,
                                    "cb_controller": cb_cont,
                        ...
                        while int(time.time() * 1000) < log_start_time + (experiment_duration * 1000):
                            ...
                            time.sleep(5)
                            # func.delete_retrys(services_list, 2, '25ms')
                            # func.delete_same_cbs(service_list=services_list)
                        log_end_time = int(time.time()* 1000)
                        log_dict = {
                            "traffic": traffic,
