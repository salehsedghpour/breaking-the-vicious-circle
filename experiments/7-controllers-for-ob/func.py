...
def initialize_cb_controllers(service_list, pc_ver_list):
    cb_controllers = {}
    for service in service_list:
        if service != 'productcatalogservice':
            ...
            for version in pc_ver_list:
                # initialize cb controllers
                cb_controllers[service+'-'+version+'-cb'] = controllers.CB_Controller()
                cb_controllers[service+'-'+version+'-cb'].trgt_rsp_time_95 = 100
                cb_controllers[service+'-'+version+'-cb'].p = 0.9
...
def deploy_same_retrys(service_list, pc_ver_list , retry_attempt, retry_interval):
    for service in service_list:
        if service == 'productcatalogservice':
            virtual_service_crud.create_retry(service_name=service,versions=pc_ver_list, retry_attempt=retry_attempt, interval=retry_interval)
        else:
            virtual_service_crud.create_retry(service_name=service, retry_attempt=retry_attempt, interval=retry_interval)


def delete_retrys(service_list, pc_ver_list , retry_attempt, retry_interval):
    for service in service_list:
        if service == 'productcatalogservice':
            virtual_service_crud.delete_retry(service_name=service,versions=pc_ver_list, retry_attempt=retry_attempt, interval=retry_interval)
        else:
            virtual_service_crud.delete_retry(service_name=service, retry_attempt=retry_attempt, interval=retry_interval)

def deploy_same_cbs(service_list, pc_ver_list, cb):
     for service in service_list:
        if service == 'productcatalogservice':
            for version in pc_ver_list:
                destination_rule_crud.create_circuit_breaker_for_specific_version(service_name=service, service_version=version,max_requests=cb, name_space="default")
        else:
            destination_rule_crud.create_circuit_breaker(service_name=service,name_space="default", max_requests=cb)


def delete_same_cbs(service_list, pc_ver_list):
     for service in service_list:
        if service == 'productcatalogservice':
            for version in pc_ver_list:
                destination_rule_crud.delete_circuit_breaker(service_name=service,name_space="default", version=version)
        else:
...
def monitor_services(service_list, pc_ver_list, version=False):
