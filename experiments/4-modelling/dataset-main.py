...
log_file_names = glob.glob('./logs/exp-4-*')
...
def extract_load(name_string):
    match = re.search(r"load-(\d+\.\d+)", name_string)
...
def extract_interval(name_string):
    match = re.search(r'interval-(\d+)ms', name_string)
...
main_df.to_csv("./datasets/exp4-main.csv")
