...
df = pd.read_csv(functions.get_project_root()+'/logs/exp-3-cb-none-interval-25ms.csv')
...
data = df.loc[(df['traffic'] == "static-110") & (df['cb'] == 'none')]
...
for index, row in data.iterrows():
    for service in challenging_services:
        ...
        success['timestamp'] = [ii for ii in success['timestamp'] if ii <= 300]
        success['data'] = success['data'][:len(success['timestamp'])]
        circuit_broken['timestamp'] = [ii for ii in circuit_broken['timestamp'] if ii <= 300]
...
plt.tight_layout()
plt.savefig(functions.get_project_root()+'/experiments/3-immediate-backoff/result-thr-cb-none-interval-25ms.png')
